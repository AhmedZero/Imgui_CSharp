// <auto-generated>
// This file was automatically generated by Biohazrd and should not be modified by hand!
// </auto-generated>
#nullable enable
using System.Diagnostics;
using System.Runtime.CompilerServices;
using System.Runtime.InteropServices;

namespace Mochi.DearImGui.Internal
{
    [StructLayout(LayoutKind.Explicit, Size = 16)]
    public unsafe partial struct ImGuiTableSettings
    {
        [FieldOffset(0)] public uint ID;

        [FieldOffset(4)] public ImGuiTableFlags SaveFlags;

        [FieldOffset(8)] public float RefScale;

        [FieldOffset(12)] public sbyte ColumnsCount;

        [FieldOffset(13)] public sbyte ColumnsCountMax;

        [FieldOffset(14)] [MarshalAs(UnmanagedType.I1)] public bool WantApply;

        [DllImport("Mochi.DearImGui.Native", CallingConvention = CallingConvention.Cdecl, EntryPoint = "__InlineHelper62", ExactSpelling = true)]
        private static extern void Constructor_PInvoke(ImGuiTableSettings* @this);

        [DebuggerStepThrough, DebuggerHidden]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public ImGuiTableSettings()
        {
            fixed (ImGuiTableSettings* @this = &this)
            { Constructor_PInvoke(@this); }
        }

        [DllImport("Mochi.DearImGui.Native", CallingConvention = CallingConvention.Cdecl, EntryPoint = "?GetColumnSettings@ImGuiTableSettings@@QEAAPEAUImGuiTableColumnSettings@@XZ", ExactSpelling = true)]
        private static extern ImGuiTableColumnSettings* GetColumnSettings_PInvoke(ImGuiTableSettings* @this);

        [DebuggerStepThrough, DebuggerHidden]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public ImGuiTableColumnSettings* GetColumnSettings()
        {
            fixed (ImGuiTableSettings* @this = &this)
            { return GetColumnSettings_PInvoke(@this); }
        }
    }
}
